...
emitProxyTaskFunction(CodeGenModule &CGM, SourceLocation Loc,
  ...
  if (CGM.getLangOpts().OpenMPPrealloc) {
    std::string Name = "omp_task_entry_" + std::to_string(Loc.getRawEncoding());
    TaskEntry =
        llvm::Function::Create(TaskEntryTy, llvm::GlobalValue::ExternalLinkage,
                               Name, &CGM.getModule());
  } else {
...
static void emitDependData(CodeGenFunction &CGF, QualType &KmpDependInfoTy,
  ...
  for (const Expr *E : Data.DepExprs) {
    ...
    if (Data.FakeAddrReplication != nullptr) {
          CGF.EmitStoreOfScalar(Data.FakeAddrReplication, BaseAddrLVal);
    } else {
  ...
  auto &&ThenCodeGen = [this, &Data, TDBase, KmpTaskTQTyRD, &TaskArgs,
    ...
    if (D.getSingleClause<OMPReplicatedClause>() && !Data.IsLastReplicaNode) {
      ...
      if(constraint == OMPC_REDUNDANCY_CONSTRAINT_spatial || constraint == OMPC_REDUNDANCY_CONSTRAINT_spatial_temporal){
          ConstraintValue = CGF.Builder.getInt1(1);
      }
      else{
          ConstraintValue = CGF.Builder.getInt1(0);
      }
...
void CGOpenMPRuntime::emitTaskgraphCall(CodeGenFunction &CGF,
  ...
  if (NumPreallocsClause) {
   NumPreallocs = CGF.EmitScalarExpr(NumPreallocsClause->getNumPreallocs());
   NumPreallocs = CGF.Builder.CreateIntCast(NumPreallocs, CGF.Int32Ty,
                                               /*isSigned=*/false);
  }
  ...
  if (IfClause) {
    ...
    if (CondVal) {
      IfVal = CGF.Builder.CreateIntCast(CondVal, CGF.Int32Ty,
                                        /*isSigned=*/true);
    }
  }
